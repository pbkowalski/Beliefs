/*
 *   RefereeFunctionDuboisPrade.java : part of package RefereeToolbox;
...
import java.util.*;
/**
 * This class implements the Referee Function encoding the combination rule of Dubois
 * & Prade.
 *
...
public class RefereeFunctionDuboisPrade<Prop extends Lattice<Prop> >
    ...
    public ArrayList<Assignment<Prop>> refereeFunction(ArrayList<Assignment<Prop>> assignIn,
        ...
        for(i=1;i<assignIn.size();i++) fusedPropConj.and(fusedPropConj,assignIn.get(i).attribute);
        if(fusedPropConj.compareTo(fusedPropDisj)==0) {
            for(i=0;i<assignIn.size();i++) fusedPropDisj.or(fusedPropDisj,assignIn.get(i).attribute);
            finalAssignment.attribute=fusedPropDisj;
        } else finalAssignment.attribute=fusedPropConj;
